    ...
    function getLastInfo() {
        ...
        try {
            database.transaction(function (tx) {
                ...
                tx.executeSql('SELECT * FROM personTable', [], function (tx, resultSet) {

                    if (resultSet.rows.length === 0) {
                        console.log("Tabela tem 0 Registos");
                        return;
                    }
                    ...
                    for (var i = 0; i < resultSet.rows.length; i++) {
                        ...
                        person = new Object();
                        person.id = resultSet.row.item(i).id;
                        person.imageURI = resultSet.row.item(i).imageURI;
                        person.name = resultSet.row.item(i).name;
                        person.birthDate = resultSet.row.item(i).birthDate;
                        person.town = resultSet.row.item(i).town;
                        person.email = resultSet.row.item(i).email;
                        person.phone = resultSet.row.item(i).phone;
